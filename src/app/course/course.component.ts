...
export class CourseComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.lessonsPager.loadFirstPage(this.id).subscribe(
      ...
      err => this.messagesService.message('error loading first page!')
    );
  }
  ...
  previousLessonsPage() {
    this.lessonsPager.previous().subscribe(
      ...
      err => this.messagesService.message('error loading previous page!')
    );
  }
  ...
  nextLessonsPage() {
    this.lessonsPager.next().subscribe(
      ...
      err => this.messagesService.message('error loading next page!')
    );
  }
